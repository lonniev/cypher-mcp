...
VOCABULARY: list[Template] = [
    ...
    Template(
        ...
        cypher=(
            ...
            "SET f.state = $state, f.at = timestamp(), f.url = $url "
            "MERGE (f)-[:BLOCKS]->(s) "
            "RETURN f.state AS state, f.kind AS kind, f.number AS number, f.url AS url"
        ),
        param_schema={
            "repo_name": {"type": "string", "required": True, "description": "Repository name."},
            ...
            "url": {"type": "string", "required": True,
                    "description": "Canonical GitHub URL of the issue/PR, carried from the CI run "
                                   "at runtime (github.server_url + repository) — never derived from "
                                   "a hardcoded owner. Lets the graph hyperlink straight to GitHub."},
        },
...
READ_VOCABULARY: list[Template] = [
    ...
    Template(
        ...
        cypher=(
            "CALL { "
            "  MATCH (c:Capability) "
            "  WITH 'Capability' AS kind, c.name AS label, c.name AS key, '' AS repo, '' AS url, "
            "       coalesce(c.updated_at, c.authored_at, c.inferred_at) AS updated_at "
            ...
            "  UNION "
            "  MATCH (sym:Symbol) "
            "  WITH 'Symbol' AS kind, sym.fqn AS label, sym.fqn AS key, '' AS repo, '' AS url, "
            ...
            "       inv.name AS key, '' AS repo, '' AS url, inv.at AS updated_at "
            "  RETURN kind, label, key, repo, url, updated_at "
            "  UNION "
            "  MATCH (p:PatentElement) "
            "  WITH 'PatentElement' AS kind, coalesce(p.name, toString(p.ref)) AS label, "
            "       toString(p.ref) AS key, '' AS repo, '' AS url, p.updated_at AS updated_at "
            "  RETURN kind, label, key, repo, url, updated_at "
            "  UNION "
            "  MATCH (s:Service) "
            "  WITH 'Service' AS kind, s.repo_name AS label, s.repo_name AS key, "
            "       s.repo_name AS repo, '' AS url, s.created_at AS updated_at "
            "  RETURN kind, label, key, repo, url, updated_at "
            "  UNION "
            "  MATCH (f:FundingBlock) "
            "  WITH 'FundingBlock' AS kind, "
            "       (f.repo_name + ' ' + f.kind + '#' + toString(f.number) + ' — ' + f.state) AS label, "
            "       toString(f.number) AS key, f.repo_name AS repo, coalesce(f.url, '') AS url, f.at AS updated_at "
            "  RETURN kind, label, key, repo, url, updated_at "
            "} "
            "WITH kind, label, key, repo, url, updated_at "

// Avatar persistence — cypher has no set_profile tool, so the chosen
// avatar lives in localStorage keyed by npub. A change dispatches a window
// event so the Nav updates without a reload.

import { AVATAR_CHOICES } from "../components/Avatar";
import { fetchProfile } from "./nostrProfile";

export const AVATAR_EVENT = "cypher:avatar-changed";

function key(npub: string): string {
  return `cypher:avatar:${npub}`;
}
